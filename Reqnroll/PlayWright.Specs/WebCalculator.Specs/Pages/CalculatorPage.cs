...
/// </summary>
public class CalculatorPage(IPageDependencyService pageDependencyService)
{
  ...
  public async Task EnterFirstNumberAsync(string number)
  {
    ...
    //Enter text
    await (await _pageDependencyService.Page).FillAsync(FirstNumberFieldSelector, number);
  }

  public async Task EnterSecondNumberAsync(string number)
  {
    //Enter text
    await (await _pageDependencyService.Page).FillAsync(SecondNumberFieldSelector, number);
  }

  public async Task ClickAddAsync()
  {
    //Click the add button
    await (await _pageDependencyService.Page).ClickAsync(AddButtonSelector);
  }
  ...
  public async Task<string?> WaitForNonEmptyResultAsync()
  {
    ...
    await (await _pageDependencyService.Page).WaitForFunctionAsync($"document.querySelector(\"{ResultLabelSelector}\").value !== \"\"");

    // Gets the value attribute of the ResultLabelSelector
    return await (await _pageDependencyService.Page).InputValueAsync(ResultLabelSelector);;
  }
